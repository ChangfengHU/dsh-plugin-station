...
var OWN = [
  ...
  {
    name: "dsh-task-console",
    full: "ChangfengHU/dsh-task-console",
    repo: "ChangfengHU/dsh-task-console",
    owner: "ChangfengHU",
    url: "https://github.com/ChangfengHU/dsh-task-console",
    category: "agent",
    ...
    downloads: 0,
    added: "2026-09-19",
    spec: "github:ChangfengHU/dsh-task-console",
    installable: true,
    score: 100
  },
  {
  ...
  },
  {
    ...
    full: "ChangfengHU/dsh-free-search",
    repo: "ChangfengHU/dsh-free-search",
    owner: "ChangfengHU",
    url: "https://github.com/ChangfengHU/dsh-free-search",
    category: "search",
    ...
    downloads: 0,
    added: "2026-09-19",
    spec: "github:ChangfengHU/dsh-free-search",
    installable: true,
    score: 100
  },
  {
...
// src/plugins.ts
import { readFile as readFile3, rename, writeFile as writeFile3 } from "node:fs/promises";
import { join as join3 } from "node:path";
var HOST_SCOPE = "@deepseek-ai/";
async function atomicManifestWrite(path, content) {
  const temporary = `${path}.plugin-station-${process.pid}-${Date.now()}.tmp`;
  await writeFile3(temporary, content, "utf8");
  await rename(temporary, path);
}
async function detachBundleForRemoval(profileDir2, packageName) {
  const path = join3(profileDir2, "package.json");
...
async function restoreDetachedBundle(edit) {
...
var PluginStationService = class extends TypertRemoteService {
  ...
  async removePlugin(payload) {
    ...
    try {
      const detached = await detachBundleForRemoval(profileDir(this.home), name2);
