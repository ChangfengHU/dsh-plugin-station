...
describe('market catalog', () => {
  ...
  it('backs the Popular and New tabs with deterministic catalog ordering', () => {
    ...
    assert.equal(recent.entries[0]!.name, 'dsh-task-console')
  })
  ...
  it('returns the picks in their listed order, each carrying its reason', () => {
    ...
    assert.deepEqual(picks.entries.map(r => r.name), [
  ...
  it('merges our own entries without duplicating an upstream listing', () => {
    ...
    assert.equal(picks.entries.find(r => r.name === 'dsh-skill-mcp-console')!.spec,
      'github:ChangfengHU/dsh-skill-mcp-console', 'our maintained entry overrides an upstream row with the same name')
  })
})
